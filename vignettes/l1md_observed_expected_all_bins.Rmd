...
all_bins <- 1:nbins
before <- cbind(l1md_annot, as.data.frame(as.matrix(OE_mat[,all_bins]))) %>%
...
myidx <- data.frame(center = all_bins, upper = all_bins+10,
                    lower = all_bins-10)
myidx$upper[myidx$upper>nbins] <- nbins # truncate at the end
myidx$lower[myidx$lower<1] <- 1
# all 2mb windows 
...
# combine the middles
all_results <- matrix(NA, nrow = nbins, ncol = nbins)
...
dd <- bind_cols(melt(all_results) %>% dplyr::rename(id = Var2, deconvolved = value), melt(as.data.frame(as.matrix(OE_mat[,all_bins]))) %>% dplyr::rename(OE = value) %>% select(OE)) 
dd$l1md_rna <- rep(l1md_annot$l1md_rna, times=nbins)
dd$frac <- rep(l1md_annot$frac, times=nbins)
after <- dd %>%
  mutate(chrom = chro)

save(before, after, file = paste0('l1md_decon_all_bins_',chro,'.RData'))
```
