...
f2 <- f2 %>% filter(bin_pair%in%f1$bin_pair) %>% dplyr::select('chr1','s1','e1','chr2','s2','e2','bin1','bin2','pet') %>% mutate(label = 'lps141')

loops <- loops %>% dplyr::select('chr1','s1','e1','chr2','s2','e2','bin1','bin2','cc') %>% dplyr::rename(pet=cc) %>% mutate(label='k562') %>% bind_rows(f2)
...
```{r}
myDconvolved <- dcon::deconvolve(myDcon, gamma=0.8)
```



```{r}
saveRDS(myDconvolved@results, file=paste0('dcon_results_gamma_0.8_',current_chrom,'_chunk',params$chunk,'.rds'))
```
