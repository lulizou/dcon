...
f2 <- f2 %>% filter(bin_pair%in%f1$bin_pair) %>% select('chr1','s1','e1','chr2','s2','e2','bin1','bin2','pet') %>% mutate(label = 'lps141')

loops <- loops %>% select('chr1','s1','e1','chr2','s2','e2','bin1','bin2','cc') %>% rename(pet=cc) %>% mutate(label='k562') %>% bind_rows(f2)
...
```{r make-dcon-object}
startidx <- ((params$chunk-1)*1000)+1
endidx <- params$chunk*1000
...
myDcon <- Dcon(mat = list(k562_rep1 = k562_rep1, k562_rep2 = k562_rep2, lps853_rep1 = lps853_rep1, lps853_rep2 = lps853_rep2, lps141_rep1 = lps141_rep1, lps141_rep2 = lps141_rep2),
               coords = coords,
               loops = loops %>% slice(startidx:endidx),
               hic = hic)
myDcon
```
...
```{r}
myDconvolved <- dcon::deconvolve(myDcon)
```


```{r}
saveRDS(myDconvolved@results, file=paste0('dcon_results_',current_chrom,'_chunk',params$chunk,'.rds'))
```
